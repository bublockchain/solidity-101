...
class Blockchain:
    ...
    def add_transaction(self, sender_private_key, receiver_public_key, amount):
        # Get the sender's public key
        sender_public_key = keys.get_public_key(sender_private_key)

        # Check if the sender has enough funds or if from coinbase
        if self.balance_sheet.get(sender_public_key, 0) >= amount or sender_public_key == 0:
           
            # Create the message to go into a transaction
            message = f"{sender_public_key}{receiver_public_key}{amount}"

            # Sign the message with the sender's private key
            signature = keys.sign_message(sender_private_key, message)

            # Add the transaction to the block
            transaction = Transaction(sender_public_key, receiver_public_key, amount, signature)
            self.current_transactions.append(transaction)
            
            # Update the balance sheet
            self.balance_sheet[sender_public_key] = self.balance_sheet.get(sender_public_key, 0) - amount
            self.balance_sheet[receiver_public_key] = self.balance_sheet.get(receiver_public_key, 0) + amount
    ...
    def mine_block(self):
        ...
        for transaction in self.current_transactions:
            message = f"{transaction.sender_public_key}{transaction.receiver_public_key}{transaction.amount}"
            if not keys.verify_signature(transaction.sender_public_key, message, transaction.signature):
                print("Invalid transaction signature found. Aborting mine.")
                return None

        # Get hash of last block to "chain" blocks together
        previous_hash = self.chain[-1].current_hash
        
        #Get index of our block
        index = len(self.chain)
        
        # Timestamp of when the block was mined
        timestamp = int(time.time())

        # Arbitary number used for mining to manipualte the hash
        nonce = 0

        # Calculate the hash of the block
        current_hash = self.calculate_hash(index, previous_hash, timestamp, nonce, self.current_transactions)

        # Mine the block by incrementing the nonce until the hash starts with 4 zeros
        while not current_hash.startswith("0000"):
            nonce += 1
            current_hash = self.calculate_hash(index, previous_hash, timestamp, nonce, self.current_transactions)
            #print(current_hash)

        # Create the new block and add to our "chain"
        new_block = Block(index, previous_hash, timestamp, nonce, current_hash, self.current_transactions)
        self.chain.append(new_block)

        # Clear the current transactions after mining a block
        self.current_transactions = []  
